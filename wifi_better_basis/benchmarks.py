...
def _embedding_needs_cache(N, seed=42):
    ...
    It must be prepared into the pipeline cache with make_qcd_cache.py, after which
    ...
    raise RuntimeError(
        "benchmark '4d_embedding_qcd' has no generator: it is real data. Prepare it "
        "with `python make_qcd_cache.py --n_train N --n_test N` and run without the "
...
BENCHMARKS = {
    ...
    "4d_embedding_qcd": _embedding_needs_cache,  # JetClass QCD single-class (make_qcd_cache.py)
}
