"""Prep job (run ONCE before workers): data partition, GMM fits, K selection,
...
X_dens_all = {N: G.get_train(N) for N in C.N_FIT_LIST}
print(f"pool dim={part['dim']}  wfit={len(part['X_wfit'])}  rest={len(part['REST'])}  "
      f"dens (order-free, per-N draws)={[len(v) for v in X_dens_all.values()]}")
...
for N in C.N_FIT_LIST:
    for K in C.K_LIST:
        ...
        if os.path.exists(fk):
            ...
            models[(N, K)] = th
            val_nll[(N, K)] = float(-G.gmm_logpdf(part["X_wfit"], th, K).mean())
...
for (N, K), th in models.items():
    ...
    print(f"tangent N={N:>7d} K={K:>2d}: P={G.n_params(K, part['dim'])}"
