#!/usr/bin/env python3
"""SINGLE ENTRY POINT for the 4D embedding GOF experiment (tangent composite test).
...
PARAMS = dict(
    ...
    chunk_budget_point=400_000,   # ~ N_test * items per point chunk (analytic scores!)
    chunk_budget_comp=150_000,    # composite is ~3x slower per t
...
def check_compatible(old, new):
    ...
    for key in ["n_fit_list", "k_list"]:
        if not set(old[key]) <= set(new[key]):
            problems.append(f"{key} may only gain values (partition v2 is order-free)")
...
def main():
    ...
    if a.run:
        ...
        env = os.environ.copy(); env["GOF4D_PARAMS"] = pfile
    ...
    if a.step == "prep":
        if a.local:
            ...
            out = subprocess.run(["sbatch", "--parsable",
                                  f"--export=ALL,GOF4D_PARAMS={pfile}", "prep.sbatch"],
        ...
        if a.local:
            ...
            out = subprocess.run(
                ["sbatch", "--parsable", f"--export=ALL,GOF4D_PARAMS={pfile}",
