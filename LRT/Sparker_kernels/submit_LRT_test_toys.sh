...
PY="$REPO_ROOT/LRT/Sparker_kernels/LRT_test.py"
...
# Set to true to freeze WiFi weights at central value -> tag: _frozen_weights
FIX_WIFI_WEIGHTS=false

# Set to true to profile WiFi weights freely with NO Gaussian prior -> tag: _free_weights
FREE_WIFI_WEIGHTS=false

# -------------------------
# Signal injection (EDIT THESE)
# -------------------------
# Set to true to inject a Gaussian bump (supervisor debug: do weights respond to strong signal?)
INJECT_SIGNAL=true
SIGNAL_LOC="3.0,0.0"    # 2D location of the bump (far from bimodal background centered near origin)
SIGNAL_SIGMA=0.3         # width of the injected Gaussian
SIGNAL_N=10000           # number of injected events (10% of Ntest=100000)
...
if [[ "$INJECT_SIGNAL" == "true" ]]; then
  CMD+=(--inject_signal --signal_loc "$SIGNAL_LOC" --signal_sigma "$SIGNAL_SIGMA" --signal_n "$SIGNAL_N")
fi
