...
# -----------------------------------------------------------------------
# Signal injection (for debugging weight profiling sensitivity)
# -----------------------------------------------------------------------
parser.add_argument('--inject_signal', action='store_true',
                    help="Inject a Gaussian bump into the data to test weight profiling sensitivity.")
parser.add_argument('--signal_loc', type=str, default="3.0,0.0",
                    help="Comma-separated location of injected signal (default: '3.0,0.0').")
parser.add_argument('--signal_sigma', type=float, default=0.3,
                    help="Width of injected Gaussian signal (default: 0.3).")
parser.add_argument('--signal_n', type=int, default=10000,
                    help="Number of injected signal events (default: 10000).")
...
if args.inject_signal:
    loc_str = args.signal_loc.replace(',', '_')
    run_tag += "_inject_loc%s_s%s_n%i" % (loc_str, str(args.signal_sigma), args.signal_n)
...
# -----------------------------------------------------------------------
# Signal injection (supervisor debug test)
# -----------------------------------------------------------------------
if args.inject_signal:
    signal_loc = np.array([float(x) for x in args.signal_loc.split(',')])
    assert len(signal_loc) == bootstrap_sample.shape[1], \
        f"signal_loc dim {len(signal_loc)} != data dim {bootstrap_sample.shape[1]}"
    signal_cov = (args.signal_sigma ** 2) * np.eye(len(signal_loc))
    np.random.seed(seed + 999)   # separate seed so injection is reproducible but independent
    signal_events = np.random.multivariate_normal(signal_loc, signal_cov, size=args.signal_n)
    bootstrap_sample = np.vstack([bootstrap_sample, signal_events])
    # shuffle so kernel centers sample both signal and background
    perm = np.random.permutation(len(bootstrap_sample))
    bootstrap_sample = bootstrap_sample[perm]
    print(f"Injected {args.signal_n} signal events at loc={signal_loc}, sigma={args.signal_sigma}")
    print(f"Combined dataset size: {bootstrap_sample.shape[0]}")
# -----------------------------------------------------------------------
