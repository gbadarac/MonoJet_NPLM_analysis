...
Lives in the top-level <repo>/data/ (a sibling of shared/), next to BOTH the raw
...
<repo>/data/<data-dir>/ZJetsToNuNu_*.npz, all label 0, none used in any training
step. The embedding is the `embeddings` key (== table[:, 1:]).
...
Writes a sibling cache dir under this same data/ folder (d flows through from the
array shape -- no downstream code change needed):
    <repo>/data/<benchmark>_Ntrain<Ntr>_Ntest<Nte>_seed<seed>/
...
Then point the kernel submit script (submit_SparkerKernels.sh) DATA_PATH at
    <repo>/data/4d_gaussian_embedding_qcd_Ntrain100000_Ntest100000_seed42/data_train.npy
"""
