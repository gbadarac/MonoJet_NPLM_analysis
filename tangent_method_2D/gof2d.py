"""Core library for the 2D toy GOF experiment (tangent-space composite test).

Truth: x1 ~ 0.5 N(-0.70,0.12^2) + 0.5 N(-0.30,0.12^2)  (bimodal), independent of
x2 ~ SkewNormal(alpha=8, xi=1, omega=0.75). Analytic truth: no data files, no
partition constraints. Model: general-d GMM (d=2) with ANALYTIC scores.

Same tangent machinery as gof4d_slurm, with ANALYTIC GMM scores (closed-form
...
# ---------------- analytic truth -------------------------------------------------
from scipy.stats import norm as _norm, skewnorm as _skewnorm

XI, OMEGA, ALPHA_SKEW = 1.0, 0.75, 8.0
DIM = 2

def sample_true(n, rng, eps=0.0):
    """Truth sample; eps != 0 inflates the skew-normal scale (power studies)."""
    comp = rng.random(n) < 0.5
    x1 = np.where(comp, rng.normal(-0.70, 0.12, n), rng.normal(-0.30, 0.12, n))
    x2 = _skewnorm.rvs(ALPHA_SKEW, loc=XI, scale=OMEGA*(1.0 + eps),
                       size=n, random_state=rng)
    return np.column_stack([x1, x2])

def true_marginal(axis, grid):
    if axis == 0:
        return 0.5*_norm.pdf(grid, -0.70, 0.12) + 0.5*_norm.pdf(grid, -0.30, 0.12)
    return _skewnorm.pdf(grid, ALPHA_SKEW, loc=XI, scale=OMEGA)
